...
;; from /home/rett/dev/cmu-perq/scheme48/vm-scratchpad/clispsrc/clisp-1994-01-08/src/compiler.lisp

(in-package cross)
(pushnew 'english *features*)
(pushnew :clisp2 *features*)
(pushnew :clisp3 *features*)
(pushnew :cross *features*)
(pushnew :compiler-debug *features*)
  ...
  (if (and (consp form) (symbolp (car form)))
      (multiple-value-bind (a b c) (fenv-search (car form) (svref env 1))
        ...
        (cond ((eq a 'system--macro) (values (funcall b form env) t))
              ...
              (t (values form nil))))
      (if (symbolp form)
          (let ((h (venv-search-macro form (svref env 0))))
            (if (symbol-macro-p h)
...
;; search a programm file. see init.lsp :
(defun search-file (filename extensions)
  (let  ((use-extensions (null (pathname-type filename))))
    (when use-extensions
...
;; siehe DEFS1.LSP :
(defconstant *compiler-date-format*
  "~1{~5@*~D/~4@*~D/~3@*~D ~2@*~2,'0D.~1@*~2,'0D.~0@*~2,'0D~:}")

(defun line-number (stream) nil)
...
(defconstant *jmpbuf-size* 256)
;; (progn
;;   (format *query-io*  "~%Please input *jmpbuf-size*: ")
;;   (read *query-io*)))

(defconstant *big-endian* nil)
      ...
      (if arg
        ...
        (let ((obj (let ((*read-base* 16.)) (read stream t nil t))))
          (unless (= (length obj) arg)
            (error  "Bad length of closure vector: ~S"
                   arg))
          obj)
        ; Closure lesen
...
(RETURN-FROM-I k n)           Verlasse den Block, dessen Block-Cons
...
(GO-I k n l)                  Springe im Tagbody, dessen Tagbody-Cons
...
(defconstant instruction-table
  '#(; (1) Konstanten
     ...
     ; (10) BLOCK
     (BLOCK-OPEN NL) (BLOCK-CLOSE O) (RETURN-FROM N) (RETURN-FROM-I NN)
     ; (11) TAGBODY
...
(defconstant short-code-base 155)
(defconstant short-code-opsize '#(15   25   21   30   10))
(defconstant short-code-ops '#(155  170  195  216  246));256
...
(defconstant funtab
  '#(system--%funtabref system--subr-info
     ...
     system--%make-closure system--make-load-time-eval clos--std-instance-p
     clos--allocate-std-instance clos-slot-value clos--set-slot-value
...
(defun %funtabref (index)
  (if (and (<= 0 index) (< index (length funtab)))
      (svref funtab index)
      nil))
  ...
  (values-list
   (assoc sym
          '(; Das ist die Tabelle aller SUBRs, wie in SUBR.D.
            ...
            (system--%in-package 1 0 nil (:nicknames :use) nil)
            ...
            #+(or UNIX OS/2) (make-pipe-input-stream 1 0 nil nil nil)
            #+(or UNIX OS/2) (make-pipe-output-stream 1 0 nil nil nil)
...
(defun compiler-error (caller &optional where)
  (error  "Compiler bug!! Occurred in ~A~@[ at ~A~]."
         caller where))
...
; Um den SP von stackz1 zu stackz2 hochzusetzen, reicht also ein (SKIPSP k).
(defun spdepth-difference (stackz1 stackz2 &aux (k 0))
  (loop
    (when (eq stackz1 stackz2) (return))
    ...
    (let ((item (car stackz1)))
      (if (consp item)
        (case (first item)
          (TAGBODY (incf k (+ 1 *jmpbuf-size*))))
        (case item
          ((PROGV MVCALLP MVCALL ANYTHING) (incf k 1))
          ((CATCH UNWIND-PROTECT BLOCK) (incf k (+ 2 *jmpbuf-size*)))
          (CLEANUP (incf k 3)))))
    (setq stackz1 (cdr stackz1)))
  k)
...
;; should work on something like this: (fenv-search 'lambda nil)
...
(defstruct (compiler-block (:copier nil))
...
(defstruct (compiler-tagbody (:copier nil))
...
; Gestalt.
(defconstant specdecl (cons nil nil))
...
; Ergebnis ist ein Symbol-Macro-Objekt genau dann, wenn v ein Symbol-Macro
; darstellt.
(defun venv-search-macro (v &optional (venv *venv*))
  (multiple-value-bind (a b c) (venv-search v venv)
    (case a
      ((NIL) (and (boundp v) (symbol-value v)))
      ((LOCAL) (svref b c))
      (t nil))))
...
(defparameter *declaration-types*
  '(special ; Bindungen
    ...
    sys::ignorable)) ; markiert Variablen als vielleicht ignorierbar
...
;   IGNORE, SYSTEM--IGNORABLE : wird weggelassen, stattdessen bei der
...
(defun c-error (cstring &rest args)
  (setq cstring
    (concatenate 'string #+DEUTSCH "~%ERROR~@[ in Funktion ~S~]~A :~%"
  ...
  (let ((in-function
    ...
    (apply #'format *c-error-output* cstring in-function (c-source-location) args))
  (throw 'c-error
...
(defun make-special-var (symbol)
  (make-var :name symbol :specialp t
            ...
            :constant (if (c-constantp symbol)
                        (make-const :value (c-constant-value symbol)
                                    :form symbol))))
 ...
 (let
  ((anode
    (catch 'c-error
      (if (atom *form*)
        (cond ((symbolp *form*)
               (let ((h (venv-search-macro *form* *venv*)))
                 (if (symbol-macro-p h) ; Symbol-Macro ?
                   (c-form (sys::%record-ref h 0)) ; -> expandieren
                   (c-VAR *form*))))
...
(defun make-funname-const (name)
  (if (atom name)
    ...
    (let ((symbol (second name)))
      (make-const :value (system--get-setf-symbol symbol)
             ...
             (case fun ; fun ein SUBR
               (; Seiteneffektklasse (NIL . NIL) haben diejenigen Funktionen,
                ...
                (SYSTEM--%FUNTABREF
                 ...
                 REALP COMPLEXP STREAMP RANDOM-STATE-P READTABLEP HASH-TABLE-P PATHNAMEP CHARACTERP
                 FUNCTIONP PACKAGEP ARRAYP SIMPLE-ARRAY-P BIT-VECTOR-P VECTORP SIMPLE-VECTOR-P
                 SIMPLE-STRING-P SIMPLE-BIT-VECTOR-P SYSTEM--SYMBOL-MACRO-P CLOS--STD-INSTANCE-P
                 ...
                 INTEGER-LENGTH LDB LDB-TEST MASK-FIELD DPB DEPOSIT-FIELD ! EXQUO) ; alle diese sind SUBRs ohne Keyword-Parameter
                (setq foldable t)
                '(NIL . NIL))
               ((VECTOR MAKE-STRING
               ...
               ((SYSTEM--SUBR-INFO
                 ...
                 SYSTEM--%COPY-STRUCTURE SYSTEM--%STRUCTURE-TYPE-P SYSTEM--CLOSURE-NAME
                 SYSTEM--CLOSURE-CODEVEC SYSTEM--CLOSURE-CONSTS SYSTEM--MAKE-CODE-VECTOR
                 SYSTEM--%MAKE-CLOSURE SYSTEM--MAKE-LOAD-TIME-EVAL SYSTEM--MAKE-SYMBOL-MACRO
                 CLOS--ALLOCATE-STD-INSTANCE CLOS-SLOT-EXISTS-P
...
(defun c-GLOBAL-FUNCTION-CALL (fun) ; fun ist ein Symbol oder (SETF symbol)
  ...
  (when *compiling-from-file* ; von COMPILE-FILE aufgerufen?
    (unless (or (fboundp fun) (member fun *known-functions* :test #'equal))
      (pushnew fun *unknown-functions* :test #'equal))
...
(defun inline-callable-function-p (form n &optional (more nil))
...
(defun process-fixed-var-list (symbols &optional optimflags)
  (do ((symbolsr symbols (cdr symbolsr))
    ...
    (push (make-var :name (gensym) :specialp nil :constantp nil
                    :usedp nil :really-usedp (null (car optimflagsr))
                  ...
                  #'(lambda (block)
                      (prog1
                        `(,(if (member block (fnode-Blocks *func*) :test #'eq)
                             `(BCONST ,block)
                             `(GET ,(compiler-block-consvar block) ,*venvc* ,*stackz*)
   )
                           (PUSH))
                        (setq *stackz* (cons 1 *stackz*))))
                  (fnode-Blocks fnode))
...
(defun c-LOAD-TIME-VALUE ()
  ...
  (let ((form (second *form*))) ; ignoriere read-only-p
    (make-anode :type 'LOAD-TIME-VALUE
                ...
                :seclass '(NIL . NIL)
                :code (if *for-value*
                        `((CONST ,(make-const :value (eval form) :form form)))
                        '()))))
...
(defun setqlist-macrop (l)
  (do ((l l (cddr l)))
      ((null l) nil)
    (let ((s (car l)))
      (when (and (symbolp s) (symbol-macro-p (venv-search-macro s))) (return t)))))
...
(defun c-BLOCK ()
  ...
  (let ((name (second *form*)))
    ...
    (let* ((*stackz* (cons 'BLOCK *stackz*)) ; Block-Frame
           (label (make-label *for-value*))
           (block (make-compiler-block :fnode *func* :label label
      ...
      (if (compiler-block-used-far block)
    ...
    (let ((a (benv-search name)))
      (cond ((null a) ; dieser Blockname ist unsichtbar
            ...
            ((block-p a) ; in *benv* ohne %benv% sichtbar
             (let ((anode (c-form (third *form*) (compiler-block-for-value a))))
               (if (and (eq (compiler-block-fnode a) *func*)
                        (may-UNWIND *stackz* (cdr (compiler-block-stackz a))))
                 ; selbe Funktionen
                 (make-anode
                   :type 'RETURN-FROM
                   :sub-anodes (list anode)
                   :seclass '(T . T)
                   :code `(,anode
                           (UNWIND ,*stackz* ,(cdr (compiler-block-stackz a)) ,(compiler-block-for-value a))
                           (JMP ,(compiler-block-label a))))
                 ...
                 (progn
                   (unless *no-code*
                     ...
                     (do ((fnode *func* (fnode-enclosing fnode)))
                         ((eq fnode (compiler-block-fnode a)))
                       (pushnew a (fnode-blocks fnode)))
                     (setf (compiler-block-used-far a) t))
                   (make-anode
                     :type 'RETURN-FROM
                     :sub-anodes (list anode)
                     :seclass '(T . T)
                     :code `(,anode
                             ,@(if (not (compiler-block-for-value a)) '((VALUES0)))
                             (RETURN-FROM ,a
                              ,@(if (eq (compiler-block-fnode a) *func*) `(,*stackz*) '()))))))))
            ...
            (t (compiler-error 'c-RETURN-FROM))))))) ; macrolet
...
(defun c-MACROLET (&optional (c #'c-form))
  ...
  (do ((L1 (second *form*) (cdr L1))
    ...
    (let* ((macrodef (car L1))
           (name (car macrodef)))
      (push name L2)
      (push (cons 'SYSTEM--MACRO (make-macro-expander macrodef))
            L2))))
                  ...
                  (let ((body-anode
                    ...
                    (let* ((codelist
                             `(,label
                               ...
                               ,body-anode
                               (UNWINDSP ,*stackz* ,*func*) ; ein (SKIPSP k)
...
(defun c-APPLY ()
  ...
  (let* ((funform (second *form*))
    ...
    (if (inline-callable-function-p funform n t)
      ; Aufruf eines Lambda-Ausdrucks INLINE möglich
      (return-from c-APPLY
              ...
              (multiple-value-bind (a b c) (fenv-search fun)
                ...
                (case a
                  ((NIL) ; globale Funktion
                    (unless (and (symbolp fun) (or (special-form-p fun) (macro-function fun))) ; Special-Form oder globaler Macro ?
                      ...
                      (let ((inline-lambdabody
                        ...
                        (if (and #| inline-lambdabody |#
                                 (consp inline-lambdabody)
                                 (inline-callable-function-p `(FUNCTION (LAMBDA ,@inline-lambdabody)) n t)
    )
                          ; Aufruf einer globalen Funktion INLINE möglich
                          (return-from c-APPLY
                            ...
                            (return-from c-TYPEP
                              (let ((*form* `((lambda ,@lambdabody) ,objform)))
                                (c-FUNCALL-INLINE
                                  ...
                                  lambdabody
                                  nil))))))))
              ((and (clos--class-p type) (eq (get (clos-class-name type) 'CLOS--CLASS) type))
                (return-from c-TYPEP
...
JMPHASH, RETURN-FROM, GO, RET, THROW, BARRIER). Die Teile stecken (jeweils als
...
(UNWINDSP stackz1 stackz2)       eine Folge von (SKIPSP k)
...
(RETURN-FROM block stackz)       (RETURN-FROM-I k n)
(TAGBODY-OPEN const label1 ... labelm)
                                 (TAGBODY-OPEN n label1 ... labelm)
(GO const l)                     (GO n l)
(GO tagbody l)                   (GO n l)
(GO tagbody l stackz)            (GO-I k n l)
(HANDLER-OPEN const stackz label1 ... labelm)
...
(defun kconst-index-1 (const)
  (kconst-index (const-value const) (const-form const)))

; (make-const-code value form) liefert den Code, der das Objekt value,
; entstanden aus form, als 1 Wert nach A0 bringt.
    ...
    (if (atom item)
      ...
      (unless *dead-code* ; nur erreichbarer Code braucht verarbeitet zu werden
        ...
        (case (first item)
                  ...
                  (if (var-constantp var)
                    (let* ((const (var-constant var))
                      ...
                      (if (fnode-p val)
                        ...
                        (make-const-code val (const-form const))))
                    (progn
          ...
          (BIND
            (push `(BIND ,(kconst-index-1 (second item))) *code-part*)
          ...
          (CALL
            (push `(CALL ,(second item) ,(kconst-index-1 (third item)))
                   *code-part*)
            (setq *current-value* nil *current-vars* '()))
          ((CALL0 CALL1 CALL2)
            (push `(,(first item) ,(kconst-index-1 (second item)))
                  *code-part*)
            (setq *current-value* nil *current-vars* '()))
          ((FUNCALLP APPLYP)
            (push '(PUSH) *code-part*)
          ...
          (BLOCK-OPEN
            (let ((label (third item)))
              (push `(BLOCK-OPEN ,(kconst-index-1 (second item)) ,label)
                     *code-part*)
          ...
          (RETURN-FROM
            (push
              (if (cddr item)
                (multiple-value-bind (k n)
                    (zugriff-in-stack (third item) (compiler-block-stackz (second item)))
                  `(RETURN-FROM-I ,k ,n))
                (if (block-p (second item))
                  `(RETURN-FROM ,(bconst-index (second item)))
                  `(RETURN-FROM ,(kconst-index-1 (second item)))))
              *code-part*)
            (finish-code-part)
            (setq *dead-code* t))
          (TAGBODY-OPEN
            (push `(TAGBODY-OPEN ,(kconst-index-1 (second item)) ,@(cddr item))
                  *code-part*)
            ...
            (push
              (if (cdddr item)
                ...
                (if (tagbody-p (second item))
                  ...
                  `(GO ,(kconst-index-1 (second item)) ,(third item))))
              *code-part*)
            (finish-code-part)
            (setq *dead-code* t))
          ...
          (HANDLER-OPEN
            (setq item
              (let ((v (const-value (second item)))
                ...
                `(HANDLER-OPEN ,(const-index (cons v k) nil) ,v ,k ,@(cdddr item))))
            (push item *code-part*)
          ...
          (when mitte
            ...
            (let ((old-for-value (get mitte 'for-value)))
              ...
              (when (and (not (eq for-value old-for-value))
                ...
                (setf (get mitte 'for-value) for-value result mitte))))) ; end let*
        (unless modified (return)))) ; end let, loop
    (let (codelistr)
       ...
       (block value
         (loop ; Codeliste durchlaufen
           ...
           (case (first (car code))
             ...
             ((T CONST CONS LIST LIST*) ; produzieren Wert /= NIL
             ...
             ((UNBIND1 SKIP SKIPI SKIPSP STORE STOREI STOREV STOREC STOREIC SETVALUE
               VALUES1 BLOCK-CLOSE TAGBODY-CLOSE CATCH-CLOSE UNWIND-PROTECT-CLEANUP)) ; keine Änderung des 1. Werts -> weiter in der Codeliste
            ...
            (loop
              ...
              (case (first (car cr2))
                ((UNBIND1 SKIP SKIPI SKIPSP VALUES1 BLOCK-CLOSE TAGBODY-CLOSE
                  CATCH-CLOSE UNWIND-PROTECT-CLEANUP) ; diese Operationen brauchen keine Werte und lassen
    ...
    (let ((item (car codelistr)))
      (when (consp item)
        (case (first item)
          ...
          (CONST
            (setf (car codelistr) `(LOADV 0 ,@(cdr item))))
          (VENV
...
(defun signature (closure)
  (let ((const-list (closure-consts closure))
    ...
    (macrolet ((pop2 (listvar)
      ...
      (progn (pop byte-list) (pop byte-list))
...
(defun compile-toplevel-form (form &optional (*toplevel-name* *toplevel-name*))
  ...
  (catch 'c-error
    ...
    (if (atom form)
      (when (symbolp form)
        (let ((h (venv-search-macro form *venv*)))
          (when (symbol-macro-p h) ; Symbol-Macro ?
            (return-from compile-toplevel-form
              (compile-toplevel-form (sys::%record-ref h 0)))))) ; -> expandieren
      (let ((fun (first form)))
        (when (symbolp fun)
          (multiple-value-bind (a b c) (fenv-search fun)
            (declare (ignore b c))
    ...
    (when (and (not *toplevel-for-value*) (c-constantp form))
      (return-from compile-toplevel-form))
    (let ((*package-tasks* '()))
      (setq form
...
(defun compile-file (file &key (output-file 'T) listing
  ...
  (with-open-file (istream file :direction :input-immutable)
    (let ((listing-stream (if new-listing-stream
                            ...
                            (if (streamp listing) listing nil))) ) ; ein Stream oder NIL
      (unwind-protect
        (let ((*compile-file-pathname* file)
              ...
              (*fasoutput-stream* (if new-output-stream
                                    ...
                                    (if (streamp output-file) output-file nil))) ; ein Stream oder NIL
              (*liboutput-stream* (if new-output-stream
                                    (open liboutput-file :direction :output)
                                    nil)) ; ein Stream oder NIL
          ...
          (unwind-protect
            (progn
              ...
              (let ((*compiling* t)
                ...
                (loop
                  ...
                  (setq *compile-file-lineno1* (compile-line-number istream))
                  (let ((form (read istream nil eof-value)))
                    (setq *compile-file-lineno2* (compile-line-number istream))
            ...
            (when new-output-stream
              (close *fasoutput-stream*) (close *liboutput-stream*)
...
(defun disassemble-closure (closure &optional (stream *standard-output*))
  ...
  (multiple-value-bind (req-anz opt-anz rest-p key-p keyword-list allow-other-keys-p
    ...
    (format stream  "~%~S required arguments"
                   req-anz)
    (format stream  "~%~S optional arguments"
                   opt-anz)
    (format stream  "~%~:[No rest parameter~;Rest parameter~]"
                   rest-p)
    (if key-p
      (let ((kw-count (length keyword-list)))
        (format stream  "~%~S keyword parameter~:P: ~{~S~^, ~}."
                       kw-count keyword-list)
        (when allow-other-keys-p
          (format stream  "~%Other keywords are allowed.")))
      (format stream  "~%No keyword parameters"))

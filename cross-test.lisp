...
(defun test-empty-function ()
  (declare (compile)))

(defun test-plus-1 (x y)
  (declare (compile))
  (+ x y))

(defun test-plus-2 (x y)
  (declare (compile))
  (test-plus-1 x y))

(defun test-empty-block ()
  (declare (compile)))

;;

(test generate-artifacts
  (generate-artifacts #'test-plus-2)
  (is (equal *current-disassembly*
             '((O 1 174) (O 1 174) (N 2 47 0) (N 2 25 3))))
  (is (equal *current-lap-assembly* '(174 174 47 0 25 3))))

(run 'generate-artifacts)

(explain! (run 'generate-artifacts))
